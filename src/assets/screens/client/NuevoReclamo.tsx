  ...
  return (
    <div className="space-y-7">
        ...
        <div>
          ...
          <Field icon={Calendar}>
            <input type="date" value={data.date} max={new Date().toISOString().split('T')[0]}
              onChange={(e) => setData({ ...data, date: e.target.value })} className={inputCls} />
          </Field>
        </div>
        <div>
      ...
      <div>
        ...
        <Field icon={MapPin}>
          <input type="text" placeholder="Dirección exacta del suceso" value={data.location}
            onChange={(e) => setData({ ...data, location: e.target.value })} className={inputCls} />
        </Field>
      </div>

      {/* Description */}
      <div>
        <label className="text-[10px] font-semibold uppercase tracking-widest text-slate-500 mb-1.5 block">Descripción de los hechos</label>
        <Field icon={FileText}>
          <textarea rows={4} placeholder="Describe brevemente lo ocurrido..." value={data.description}
...
/* ─── Step 1: Photos ── */
const StepPhotos = ({ files, setFiles, onNext, onBack }: { files: File[]; setFiles: (f: File[]) => void; onNext: () => void; onBack: () => void }) => (
  <div className="space-y-7">
    <div>
...
/* ─── Step 2: AI Analysis ── */
const StepAI = ({ files, data, onNext, onBack }: { files: File[]; data: ClaimData; onNext: () => void; onBack: () => void }) => {
  ...
  useEffect(() => {
    const runAnalysis = async () => {
      try {
        ...
        if (resData.success) { setResult(resData.analysis); setPhase(2); }
        else throw new Error(resData.msg || 'Error de IA');
      } catch (error: unknown) {
      ...
      <div className={`rounded-3xl p-8 text-white relative overflow-hidden transition-all duration-500
        ...
        <div className="relative z-10 flex flex-col items-center gap-3 text-center">
          {phase === 1
            ...
            : isFraude ? <ShieldAlert size={44} /> : <ShieldCheck size={44} />}
          <p className="text-lg font-bold">
            {phase === 1 ? 'Analizando píxeles...' : isFraude ? 'Riesgo detectado' : 'Evidencia validada'}
          </p>
          {phase === 1 && (
            <div className="flex items-center gap-2 text-white/40">
              <Loader2 size={12} className="animate-spin" />
              <span className="text-[10px] font-semibold uppercase tracking-widest">Procesando con IA...</span>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 space-y-3">
            ...
            <div className="flex items-center justify-between pt-2 border-t border-slate-100">
              ...
              <span className={`text-xs font-bold ${isFraude ? 'text-red-500' : 'text-emerald-500'}`}>
